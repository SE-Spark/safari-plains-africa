...
final class GroupTable extends PowerGridComponent
{
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ->addColumn('name')
            ->addColumn('status', fn(Group $model) => $model->status==1?'Active':'Inactive')
    ...
    public function actions(\App\Models\Group $row): array
    {
        return [
            Button::add('edit')
                ...
                ->dispatch('editGroup', ['groupId' => $row->id]),
            Button::add('delete')
                ->slot('<i class="bi bi-trash-fill text-danger"></i>')
                ->id()
                ->class('btn btn-transparent')
                ->dispatch('deleteGroup', ['groupId' => $row->id]),
        ];
    }
